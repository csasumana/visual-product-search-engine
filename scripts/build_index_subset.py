...
SUBSET_METADATA_PATH = "data/metadata_subset_25k.csv"

# Output paths for subset build
SUBSET_EMBEDDINGS_PATH = "embeddings/image_embeddings_25k.npy"
SUBSET_EMBEDDINGS_FP16_PATH = "embeddings/image_embeddings_25k_fp16.npy"
SUBSET_EMBEDDING_MAP_PATH = "embeddings/embedding_map_25k.csv"
SUBSET_GLOBAL_INDEX_PATH = "indexes/global_25k.index"
SUBSET_INDEX_META_PATH = "indexes/index_meta_25k.json"
SUBSET_CATEGORY_INDEX_DIR = "indexes/category_indexes_25k"
...
def ensure_directories():
    ...
    settings.resolve_path(SUBSET_CATEGORY_INDEX_DIR).mkdir(parents=True, exist_ok=True)


def load_metadata() -> pd.DataFrame:
    metadata_path = settings.resolve_path(SUBSET_METADATA_PATH)
    ...
    if missing:
        raise ValueError(f"Subset metadata missing required columns: {missing}")
...
def build_embeddings(df: pd.DataFrame, clip_service: CLIPService):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Embedding subset images"):
    ...
    if not all_embeddings:
        raise RuntimeError("No embeddings were generated. Check your subset metadata.")
...
def save_embeddings(embeddings: np.ndarray, valid_df: pd.DataFrame):
    embeddings_path = settings.resolve_path(SUBSET_EMBEDDINGS_PATH)
    np.save(embeddings_path, embeddings)
    print(f"[INFO] Saved float32 embeddings: {embeddings_path}")

    if SAVE_FP16_COPY:
        embeddings_fp16 = embeddings.astype(np.float16)
        fp16_path = settings.resolve_path(SUBSET_EMBEDDINGS_FP16_PATH)
    ...
    embedding_map_path = settings.resolve_path(SUBSET_EMBEDDING_MAP_PATH)
    embedding_map.to_csv(embedding_map_path, index=False)
    print(f"[INFO] Saved embedding map: {embedding_map_path}")

    failed_path = settings.resolve_path("data/failed_images_25k.csv")
    return failed_path


def build_global_index(embeddings: np.ndarray):
    print("[INFO] Building subset FAISS index...")
    index = FAISSService.build_index(embeddings)
    global_index_path = settings.resolve_path(SUBSET_GLOBAL_INDEX_PATH)
    FAISSService.save_index(index, global_index_path)
    print(f"[INFO] Saved subset global index: {global_index_path}")
    return index
...
def build_category_indexes(embeddings: np.ndarray, valid_df: pd.DataFrame):
    ...
    print("[INFO] Building subset category-specific FAISS indexes...")
    built_categories = []
    ...
    for category, group_df in grouped:
        ...
        cat_index_path = settings.resolve_path(SUBSET_CATEGORY_INDEX_DIR) / f"{safe_name}.index"
        FAISSService.save_index(cat_index, cat_index_path)
        built_categories.append(category)

    print(f"[INFO] Built {len(built_categories)} subset category indexes.")
...
def save_index_metadata(embeddings: np.ndarray, valid_df: pd.DataFrame, built_categories: List[str]):
    ...
    index_meta_path = settings.resolve_path(SUBSET_INDEX_META_PATH)
    with open(index_meta_path, "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)

    print(f"[INFO] Saved subset index metadata: {index_meta_path}")
...
def main():
    ...
    print("[INFO] Loading subset metadata...")
    df = load_metadata()
    ...
    print(f"Global index: {settings.resolve_path(SUBSET_GLOBAL_INDEX_PATH)}")
